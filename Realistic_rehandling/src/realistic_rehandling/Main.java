        ...
        try {
            try {
                in = new BufferedInputStream(new FileInputStream(
                        rootpath +
                        ":\\program\\location assignment\\data_realistic\\config.properties"));
                p.load(in);
...
class Dynamic_programming {
    ...
    int[] stacking_configuration;
    double[][] estimate_rehandling_times;

    void input_rehandle(String filename){
       ...
       stacking_configuration = new int[dataarray.length];
       estimate_rehandling_times = new double[dataarray.length][3];
       for(int i=0; i<size; i++){
           stacking_configuration[i] = Integer.valueOf(dataarray[i][0]);
           for(int j=0; j<3; j++)
               estimate_rehandling_times[i][j] = Double.valueOf(dataarray[i][j+1]);
       }

    }

    double calculate_rehandle(int sc, int gn){
        double rehandle=0.0;
        int index =0;
        for(int i=0; i<stacking_configuration.length; i++)
            if(stacking_configuration[i]==sc){
                index = i;
                break;
            }
        if(gn==100)
           rehandle =  estimate_rehandling_times[index][0];
        if(gn==10)
           rehandle =  estimate_rehandling_times[index][1];
        if(gn==1)
           rehandle =  estimate_rehandling_times[index][2];
   ...
   double calculate_value(int t, String nc, int[] sc, int[] gn, String[] nc_pre, int[][] sc_pre, double[] va_pre, double[] ratio){
       ...
       state_weight_chosen = new int[gn.length];
        ...
        for(int i=0; i<gn.length; i++){ //gn.length kinds of arrival
            double rehandle_times_temp = Double.MAX_VALUE;
            for(int j=0; j<nc.length(); j++){ //there are at most nc.length possible locations
                if(nc.charAt(j)>'0'){
                    ...
                    double rehandle_times =calculate_rehandle(sc_temp[j], gn[i]);
...
*/
    void dynamic_programming(int s, int t, int[] gn, double[] ratio){
        ...
        input_rehandle("rehandling times.txt"); //input expected estimate rehandling times
            // s: the number of stacks
        // t: the number of tiers
        // g: the number of weight groups
        int n=0;
    ...
    void evaluate_function_simplified(int s, int t, String[] gn){
        ...
        for(int i=0; i<string_generate.length; i++){
            ...
            for(int j=0; j<target.length;j++){
                ...
                //changes the stacking state of the yard bay
                int stack_no = 0;
                for(int k=0; k<s; k++){
                    if(bay_state[k][0]==dataarray.state_number_chosen[target_index][target_k])
                        if(bay_state[k][1]==dataarray.state_weight_chosen[target_index][target_k]){
                ...
                if(target_k==gn.length-4)
                    position[j][0]=1000;
                position[j][1] = stack_no;
                position[j][2] = dataarray.state_number_chosen[target_index][target_k]; //could be see the tier number. the larger the number is, the lower the tier.
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //for the case with two weight groups.
